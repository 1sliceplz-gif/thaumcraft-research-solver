# solver_graph.py
"""
Aspect graph construction, tiers, and aspect-chain search.
"""
...
# Frozen adjacency for fast lookups
...
def _compute_tiers():
    """
    Compute 'tier' for each aspect based on distance from primals.
    Primals and unknown aspects get tier 0; others are 1 + max(parent tiers).
    """
    memo = {}
...
def calculate_aspect_weights(prefer_ordo=True):
    """
    Assign weights to aspects for pathfinding:
      - Primals: base weight 1
      - Ordo: weight 0 (always)
      - Instrumentum: weight 0 if prefer_ordo is True
      - Others: tier + 1
    Cached per prefer_ordo value.
    """
...
# Default global weights (prefer Ordo/Instrumentum)
...
def clear_chain_cache():
    """Clear the internal cache for aspect chains."""
    _chain_cache.clear()


def _find_chain_dijkstra(start_asp, end_asp, length, weights):
    """
    Find a chain of aspects of *exact* given length (tiles) between
    start_asp and end_asp, minimizing total intermediate weight.

    Length N means N-1 edges in aspect graph.
    """
    if length < 2:
        return None
    ...
    # Simple 2-step chain: just check direct adjacency
    if length == 2:
        if end_asp in _GRAPH_FROZEN.get(start_asp, _EMPTY_FSET):
            return [start_asp, end_asp]
        return None

    target_steps = length - 1  # number of edges
    # (cost, steps, current_aspect, path_list)
    ...
    while heap:
        ...
        for nbr in _GRAPH_FROZEN.get(current, _EMPTY_FSET):
            # At final step we must land on end_asp
...
def find_aspect_chain_exact(start_asp, end_asp, length, prefer_oi=True):
    """
    Public wrapper around _find_chain_dijkstra with caching.
    prefer_oi: passed through to calculate_aspect_weights.
    """
