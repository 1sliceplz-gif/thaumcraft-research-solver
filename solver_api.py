# solver_api.py
"""
GUI-facing solver API: rating, high-level solve wrapper, and debug output.
"""
...
def calculate_solve_rating(placements, prefer_oi=True):
    """
    Compute a rating score/grade based on total aspect weight used.
    Returns (total_weight, grade).
    """
    if not placements:
        return 0, "S"
    # Positional arg so parameter name mismatch (prefer_oi vs prefer_ordo)
    # doesn't matter.
...
def solve_with_center_priority(
    ...
    """
    High-level wrapper used by the GUI.

    - Clears old solver placements.
    - Validates that at least 2 user-placed aspects exist.
    - If already connected, returns success with S rating.
    - Otherwise runs the solver once (MST/spine strategies),
      applying placements directly to the provided grid.
    """
    result = SolveResult()
    emit = (
...
def debug_output_board(grid, ring_count=4):
    """
    Print a summary of board state: user-placed vs solver-placed aspects,
    usage counts, and total weight.
    """
    print("\n" + "=" * 70)
...
def debug_output_solve_result(result):
    """
    Print a high-level summary of a SolveResult and the last-solve details.
    """
    print("\n" + "=" * 70)
    print("SOLVE RESULT")
    print("=" * 70)
...
def debug_output_connections_simple():
    """
    Shorter, more compact debug output of the latest solve connections.
    """
    details = get_last_solve_details()
