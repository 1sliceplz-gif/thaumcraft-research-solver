# solver_core/solver_strategies.py
"""
Solver strategies (MST, spine, all-orderings) and main multi-strategy solver.
"""
...
def _try_mst_strategy(
    ...
    """
    Simple MST-based strategy:
      1. Build best pairwise edges between all user-placed nodes.
      2. Run Kruskal MST over those edges.
      3. Merge resulting placements into a grid.
    """
...
def _try_spine_with_pair(
    ...
    """
    Attempt a 'spine + branches' solution using user_placed[idx_a] and
    user_placed[idx_b] as the main spine endpoints.
    """
    ...
    branch_paths = []  # kept for compatibility, but not used for symmetry now
    ...
    for b_idx, idx in enumerate(remaining):
        ...
        best_branch = None
        # score: (count, weight, straightness)
        best_branch_score = (
            float("inf"),
            float("inf"),
            float("inf"),
        ...
        # Only check nearby targets instead of ALL filled cells
        ...
        # Fallback: if no nearby target found, search all targets
...
def _try_spine_strategy(
    ...
    """
    Try all user-placed pairs as possible spine endpoints and pick the best.
    """
...
def _try_all_orderings(
    ...
    """
    Exhaustively try each user-placed aspect as a 'root' and connect others
    in a heuristic order, scoring total placements and weight.
    """
    best_result = None
    best_edges = None
    best_score = (float("inf"), float("inf"))
    ...
    for root_idx in range(n_roots):
        ...
        all_placements = []
        edges_used = []
        branch_paths = []  # kept for compatibility
        success = True
        ...
        for o_idx, (other_idx, _) in enumerate(others):
            ...
            # Try connected user-placed nodes first
...
def _score_placements(placements, weights):
    """Helper to score a placement list as (count, weight)."""
    return (
        len(placements),
...
def solve_with_spine_strategy(
    ...
    """
    Try multiple strategies with adaptive slack.
    Starts with low slack for speed, increases if no solution found.
    Early-exits if a perfect (weight=0) solution is found.

    Behavior:
      - Determine all user-placed aspects.
      - Infer ring_count and select only edge user-placed aspects as
        'nodes to connect' when there are at least 2 of them.
      - Fall back to all user-placed aspects otherwise.
    """
    ...
    # Prefer to use only edge user-placed aspects as "nodes to connect"
    ...
    # Adaptive slack: try low slack first, increase if needed
    ...
    for slack_idx, current_slack in enumerate(slack_levels):
        # Adjust progress range for this slack attempt
        ...
        # Divide this slack's range across strategies
        if use_all_orders:
        ...
        if result is not None:
            ...
            # Early exit: weight 0 can't be beaten
            if best_score[1] == 0:
                _progress(1.0, "Done!")
                break
        ...
        # If we found a solution, stop trying higher slack
    ...
    _last_solve_details = {
        ...
        # store all user-placed for debug, not just anchors
        "user_placed": full_user_placed,
    }
...
def solve_mst(grid, prefer_ordo=True, extra_slack=6, progress_cb=None):
    """
    Public wrapper around solve_with_spine_strategy (kept for compatibility).
    """
...
def apply_placements(grid, placements):
    """
    Apply a list of (pos, asp) placements to the board as solver-placed.
    """
