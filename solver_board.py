# solver_core/solver_board.py
"""
Hex-grid utilities and board-level helpers (no strategies).
"""
...
def grid_ring_count(grid):
    """
    Infer the ring_count (radius) of a hex grid by looking at coordinates.
    Assumes the grid is a full hex of radius R centered at (0, 0).
    """
    max_ring = 0
...
def are_all_aspects_connected(grid):
    """
    Check if all non-void aspect tiles form a single connected component,
    using aspect-graph adjacency for connections.
    """
    aspect_positions = [
...
def compute_aspect_components(grid):
    """
    Compute connected components among all non-void aspect tiles.

    Returns:
        dict[pos] -> component_id (0, 1, 2, ...)
    Only tiles with an aspect and not voided are included.
    """
    positions = [
        pos for pos, cell in grid.items()
        if cell.get("aspect") and not cell.get("void", False)
...
def get_user_placed(grid):
    """
    Return list of (pos, aspect) for user-placed, non-void cells.
    """
    return [
        (pos, cell["aspect"])
...
def get_user_placed_positions(grid):
    """
    Return set of positions for user-placed, non-void cells.
    """
    return {
        pos for pos, cell in grid.items()
        if cell.get("aspect")
...
def is_board_solved(grid):
    """
    Board is considered 'solved' if at least 2 user-placed aspects exist
    and all aspects are connected.
    """
    return (
...
def clear_solver_placements(grid):
    """
    Remove all solver-placed aspects (keep player_placed ones).
...
def get_solve_stats(grid):
    """
    Compute simple fill/void statistics for the board.
    Returns dict: total, voided, filled, empty, fill_percent.
    """
    total = len(grid)
