# solver_core/solver_paths.py
"""
Board tile pathfinding and path-scoring utilities.
"""
...
def _path_straightness(tile_path):
    """
    Count direction changes along a tile path.
    Lower is 'straighter'.
    """
    if len(tile_path) < 3:
        return 0
...
def _path_direction_signature(tile_path):
    """
    Represent a path by its sequence of direction steps,
    e.g. [(1,0), (1,-1), ...].
    """
    if len(tile_path) < 2:
        return ()
...
def find_path_of_length(grid, start, end, length):
    """
    Fast BFS path search using tuple-based visited tracking.
    Finds a path of *exact* given length (number of tiles)
    between start and end, avoiding voids and occupied cells
    (except the end).
    """
    if length < 2:
        return None
    ...
    # queue entries: (position, steps_so_far, path_as_tuple)
    queue = deque([(start, 1, (start,))])
    # seen state: (position, steps_so_far)
    seen = {(start, 1)}
...
def _find_straightest_path(grid, start, end, length):
    """
    Find path with fewest direction changes. Uses a heap + parent pointers.
    Falls back to BFS find_path_of_length if a heap cap is exceeded.
    """
    if length < 2:
        return None
    if length == 2:
    ...
    init_state = (start, 1, None)  # (pos, steps, prev_dir)
    parent = {init_state: None}
    best_cost = {}
    best_result = None
    best_changes = float("inf")
    tiebreak = 0
    heap = [(0, 0, 1, start, None)]  # (changes, tiebreak, steps, pos, prev_dir)
    MAX_HEAP = 50_000
    ...
    while heap:
        ...
        for nbr in get_neighbors(pos):
            ...
            # Bounded cycle check via parent chain
            in_path = False
            s = state
            depth = 0
    ...
    # Fallback to BFS if nothing better found
