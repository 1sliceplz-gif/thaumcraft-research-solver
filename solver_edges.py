# solver_core/solver_edges.py
"""
Edge construction and MST helpers for connecting user-placed aspects.
"""
...
def _get_nearby_targets(temp_grid, pos_r, max_dist=8):
    """
    Return filled cells within max_dist of pos_r.
    Avoids searching distant cells that can't possibly connect.
    """
    targets = []
...
def _find_best_edge(
    ...
    """
    Two-phase approach to connect (pos_a, asp_a) to (pos_b, asp_b):

      Phase 1: For each candidate length in [min_len, min_len+extra_slack],
               use BFS board paths + Dijkstra aspect chains to find best
               edge by (placement_count, total_weight).

      Phase 2: Re-run ONLY the winning length with the straightest
               tile path, and recompute placements/score.

    Returns a dict with keys:
      - placements:   list[(pos, asp)]
      - count:        number of new placements
      - weight:       total weight of new placements
      - tile_path:    list of positions
      - aspect_chain: list of aspects along path
      - from_pos, from_asp, to_pos, to_asp
      - straightness: direction-change count
    or None if no valid connection was found.
    """
    # existing_branch_paths is accepted but ignored now (no symmetry scoring)
    base_dist = hex_distance(pos_a, pos_b)
    min_len = max(2, base_dist + 1)
    max_len = min_len + extra_slack

    # Phase 1: Fast scan with BFS paths
    best_edge = None
    best_score = None
    best_length = None
    best_chain = None
    ...
    for length in range(min_len, max_len + 1):
        # Find aspect chain of required length
        aspect_chain = _find_chain_dijkstra(asp_a, asp_b, length, weights)
        if not aspect_chain:
            continue

        # Find tile path of required length
    ...
    # Phase 2: Refine winner with straightest path
...
def build_pairwise_edges(
    ...
    """
    For each unordered pair of user-placed aspects, attempt to build the best
    connecting edge, returning a list of such edges.
    Each edge dict will later be annotated with 'a' and 'b' (indices).

    Pairs that are already connected in the current board (same component)
    are skipped.
    """
    edges = []
    n = len(user_placed)
    ...
    # Precompute components of the current board (user + solver placements)
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            # Skip pairs that are already connected in the current board
...
def build_mst(user_placed, edges):
    """
    Build a minimum spanning tree over user_placed nodes, using edges
    sorted by (count, weight). Returns list of edges in the MST, or
    None if a full tree cannot be formed.
    """
...
def merge_mst_placements(grid, mst_edges):
    """
    Apply MST edges' placements into a temp copy of the grid.
    Returns a list of (pos, asp) of all new placements, or None if
    any conflict occurs (different aspects on same cell).
    """
