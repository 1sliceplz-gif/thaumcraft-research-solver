# main.py
# Enhanced Thaumcraft Research Solver
...
clear_button   = None
solve_button   = None
credits_button = None  # NEW

size_flash_frames = 0  # how many frames to flash hex2 after size change
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            # While credits are shown, ignore other keys
            if getattr(board, "show_credits", False):
                continue

            # Block keys while solving
            ...
            # Credits button always works
            if event.button == 1 and credits_button and credits_button.collidepoint(mx, my):
                board.toggle_credits()
                continue

            # Block all other mouse if credits visible
            if getattr(board, "show_credits", False):
                continue

            # Block mouse while solving (except credits, handled above)
            if board.solve_running:
                continue
    ...
    # IO button (dim when IO chaining is OFF)
    if IO_CENTER_IMG:
        draw_centered_icon(
            io_button,
            IO_CENTER_IMG,
    ...
    # Size button: base hex1, flash/hover with hex2
    if SIZE_IMG_1:
    ...
    # Credits button styled like tooltip panel
    if credits_button:
        base_color = (30, 30, 30)
    ...
    # Draw credits overlay LAST so nothing overlaps it
    if getattr(board, "show_credits", False):
